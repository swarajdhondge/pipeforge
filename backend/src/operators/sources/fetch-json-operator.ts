...
/**
 * FetchJSONOperator - HTTP GET requests to fetch JSON data
 * 
 * Features:
...
export class FetchJSONOperator extends BaseOperator {
   ...
   * For source operators, schema is determined at runtime via preview
   * @param _inputSchema - Not used for source operators
   * @param _config - Fetch configuration
   * @returns null (schema determined at runtime via preview)
   */
  getOutputSchema(_inputSchema?: ExtractedSchema, _config?: FetchConfig): ExtractedSchema | null {
    ...
    // Schema is extracted from actual fetched data using SchemaExtractor
    return null;
  }

  /**
   * Extract schema from fetched data (used by preview)
