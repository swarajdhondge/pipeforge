...
import { createSecret, clearError } from '../../store/slices/secrets-slice';

interface CreateSecretModalProps {
  onClose: () => void;
}

export const CreateSecretModal: FC<CreateSecretModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const result = await dispatch(createSecret({
      name: name.trim(),
      description: description.trim() || undefined,
      value: value.trim(),
    }));

    if (createSecret.fulfilled.match(result)) {
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-bg-surface rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-text-primary">Create New Secret</h3>
          <button
            onClick={handleClose}
