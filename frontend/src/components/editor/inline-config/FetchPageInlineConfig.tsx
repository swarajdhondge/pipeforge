...
import { validateFetchPage } from '../../../utils/inline-validation';
import type { FetchPageConfig, SecretRef } from '../../../types/operator.types';

interface FetchPageInlineConfigProps {
  nodeId: string;
  config: FetchPageConfig;
  onConfigChange: (config: FetchPageConfig) => void;
  onValidationChange?: (isValid: boolean, errors: string[]) => void;
}

/**
 * Inline configuration component for Fetch Page operator.
 * Displays URL input, CSS selector input, attribute input, multiple toggle, and preview button.
 * 
 * Requirements: 2.2 - WHEN a Fetch operator is displayed THEN the System 
 * SHALL show URL input field and Preview button inline
 */
export const FetchPageInlineConfig: FC<FetchPageInlineConfigProps> = ({
  nodeId,
  config,
  onConfigChange,
  onValidationChange,
  ...
  const validation = useMemo(() => validateFetchPage(config), [config]);
  ...
  return (
    <div className="space-y-2 p-1 nodrag nowheel" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
      ...
      <ValidatedInput
        ...
        onClick={(e) => e.stopPropagation()}
        onMouseDown={(e) => e.stopPropagation()}
        placeholder="div.content, h1, .article-title"
        error={validation.fieldErrors.selector}
        helpText="CSS selector to extract elements from the page"
      />
      ...
      <ValidatedInput
        ...
        helpText="Extract attribute value instead of text content"
      />
      ...
      <div className="flex items-center gap-2">
        ...
        />
        <label htmlFor={`${nodeId}-multiple`} className="text-xs text-text-secondary">
          Extract all matches
        </label>
      </div>
      ...
      <button
        ...
        onMouseDown={(e) => e.stopPropagation()}
        disabled={!config.url || !config.selector || isLoading}
