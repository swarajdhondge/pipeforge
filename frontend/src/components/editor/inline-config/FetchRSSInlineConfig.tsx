...
import { validateFetchRSS } from '../../../utils/inline-validation';
import type { FetchRSSConfig, SecretRef } from '../../../types/operator.types';

interface FetchRSSInlineConfigProps {
  nodeId: string;
  config: FetchRSSConfig;
  onConfigChange: (config: FetchRSSConfig) => void;
  onValidationChange?: (isValid: boolean, errors: string[]) => void;
}

/**
 * Inline configuration component for Fetch RSS operator.
 * Displays URL input, maxItems input, and preview button.
 * 
 * Requirements: 2.2 - WHEN a Fetch operator is displayed THEN the System 
 * SHALL show URL input field and Preview button inline
 */
export const FetchRSSInlineConfig: FC<FetchRSSInlineConfigProps> = ({
  nodeId,
  config,
  onConfigChange,
  onValidationChange,
  ...
  const validation = useMemo(() => validateFetchRSS(config), [config]);
  ...
  const handleMaxItemsChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const value = parseInt(e.target.value, 10);
    onConfigChange({ ...config, maxItems: isNaN(value) ? undefined : value });
  }, [config, onConfigChange]);

  // Preview is for schema extraction only - no success toast needed (Requirement 2.1)
  const handlePreview = useCallback(async () => {
    if (!config.url) return;
    await preview('fetch-rss', config);
  ...
  return (
    <div className="space-y-2 p-1 nodrag nowheel" onClick={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
      ...
      <ValidatedInput
        ...
        max={100}
        placeholder="50"
        error={validation.fieldErrors.maxItems}
        helpText="Maximum number of feed items to fetch (1-100)"
      />
