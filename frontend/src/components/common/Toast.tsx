...
const typeConfig: Record<ToastType, { bg: string; icon: ReactNode; defaultDuration: number }> = {
  success: {
    bg: 'bg-green-500',
  ...
  error: {
    bg: 'bg-red-500',
    defaultDuration: 8000, // Auto-dismiss after 8 seconds
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
...
const ToastItem: FC<{ toast: Toast; onDismiss: (id: string) => void }> = ({ toast, onDismiss }) => {
  ...
  }, [toast.id, toast.duration, config.defaultDuration, onDismiss]);

  return (
    <div
      className={`
        ${config.bg} text-white rounded-md shadow-lg p-4 min-w-[300px] max-w-md
